...
const isTextEntered = () => {
  ...
  // #region Вывод в консоль для дебаггинга -----------------------------------------
  ...
  if (lngth === y.length && lngth === 4) {
    ...
    const wPolygone = 0.00000026517190441087 * document.getElementById("inputWidth").value
    x[iiEnd] = x[iEnd] + wPolygone * Math.cos(a[iStart] + 1.486115503)
    y[iiEnd] = y[iEnd] + wPolygone * Math.sin(a[iStart] + 1.486115503)

    console.log("--------------- a[iStart] + 1.655477151 = ", a[iStart]*180/Math.PI, 1.486115503*180/Math.PI)


    console.log('координата третьей точки от наибольшей стороны =', x[iiEnd], y[iiEnd])
    console.log('iStart, iEnd, iiEnd, iiiEnd, a[iStart] :', iStart, iEnd, iiEnd, iiiEnd, a[iStart] );
    console.log("массив относительных координат X\n" + x);
    console.log("массив относительных координат Y\n" + y);
