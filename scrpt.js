...
function polygonOrthogonalization() {
   ...
   if (polygonEdgesNumber === 4) {
      ...
      let iiEnd = (iEnd + 1) % polygonEdgesNumber;   // Определяем индексы X,Y третьей вершины
      let iiiEnd = (iiEnd + 1) % polygonEdgesNumber; // Определяем индексы X,Y четвёртой вершины 

// XXX -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- 

/* TODO     * Не важно как там определяется угол, главное то, что прибавление +90 градусов на поворот по часовой стрелке работает.
            * Вообще не нужно что бы и прямоугольные  и многоуголные и любые полигоны обрабатывать:
            * Нужно только прямоугольные
            ...
            * */
                                                                     ...
                                                                     // FIXME Определять идеальную высоту полигона нужно после определения идеального угла ужав его в визуальный
      ...
      x[iiEnd] = x[iEnd] + 1.278481 * wPolygone * Math.cos(a[iStart] - 1.5707963268); // FIXME косинус это отношение! зачем считать косинус если можно подсчитать само отношение?
      y[iiEnd] = y[iEnd] + wPolygone * Math.sin(a[iStart] - 1.5707963268); // FIXME По Y вообще коррекция не нужна - по этой оси ничего не искажается
...
function ratio(x1, y1, x2, y2) { return (y2 - y1) / ((x2 - x1)*1.278481); };
